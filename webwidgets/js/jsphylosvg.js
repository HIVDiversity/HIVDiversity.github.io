...
Smits.PhyloCanvas.NexmlParse.prototype = {
	...
	Rectangular : {
		...
		showScaleBar	: true		// (STRING,  e.g. "0.05") Shows a scale bar at the bottom of the tree
	},
...
Smits.PhyloCanvas.Render.SVG.prototype = {
	...
	recursiveCalculateNodePositions = function (node, positionX){
		...
		if(node.children.length > 0){
			...
			if(node.name){
                ...
                // add color atribute hear according to cluster
                attr["stroke"] = Smits.Common.clusterColors[
                    Smits.Common.clusterDict[node.name]];  //"#d7e3bc";
	...
	return function(sSvg, dataObject){	
		...
		scaleY = Math.round((canvasY - paddingY*2) / (sParams.showScaleBar ? 100 + node.getCountAllChildren() : 100 + node.getCountAllChildren() - 1 ) );
	...
	function recursiveCalculateNodePositions(node, positionX){
		...
		if(node.children.length > 0){
			...
			if(node.name){
                ...
                // add color atribute here, according to cluster
                attr["stroke"] = Smits.Common.clusterColors[
                    Smits.Common.clusterDict[node.name]];  
